...
class SalesforceLoginController{
  ...
  static Future<String?> getAccessTokenWithRefreshToken({required String refreshToken}) async {
    ...
    try {
      ...
      if (response.statusCode == Constants.STATUS_CODE_OK) { // Status Code 200 OK
        ...
        logger.d('4');
        await SecureFileManager.setExpiryTimeOfToken(DateTime.now().add(const Duration(minutes: Constants.TOKEN_TIMEOUT_MINUTES)).toString());  
