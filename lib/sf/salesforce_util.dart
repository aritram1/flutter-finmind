...
class SalesforceUtil{
  ...
  static String generateEndpointUrl({ required String opType, String objAPIName = '', 
    ...
    String endpointUrl = '';
    if(opType == Constants.LOGIN){ // testing completed
    ...
    if(opType == Constants.INSERT){ // testing completed
      endpointUrl = '$instanceUrl$compositeUrlForInsert$objAPIName';
    }
    else if(opType == Constants.UPDATE){ // testing completed 
      endpointUrl = '$instanceUrl$compositeUrlForUpdate';
    }
    else if(opType == Constants.DELETE){
  ...
  static Map<String, dynamic> generateBody({required String opType, String objAPIName = '', List<Map<String, dynamic>> fieldNameValuePairs = const [], List<String> recordIds = const [], int batchCount = 0}){
    ...
    if(opType == Constants.DELETE){}                    // no body element is required for delete
    else if(opType == Constants.SYNC){}                 // this is a custom API, as of now it does not require `body`
    else if(opType == Constants.DELETE_MESSAGES){}      // this is a custom API, as of now it does not require `body`
    else if(opType == Constants.APPROVE_MESSAGES){      // Approve MEssages
      Map<String, dynamic> dataMap = {};
      dataMap['data'] = recordIds;
      body['input'] = dataMap;
    }
    else if(opType == Constants.INSERT || opType == Constants.UPDATE){ // Insert or Update operation
      var allRecords = [];
      ...
      if(opType == Constants.UPDATE){
        body['allOrNone'] = 'false';
      }// if(detaildebug) log.d('body=>' + body.toString());
    }
    
    else if(opType == Constants.DELETE_TRANSACTIONS){}  // do nothing
