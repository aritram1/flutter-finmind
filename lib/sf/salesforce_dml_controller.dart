...
class SalesforceDMLController{
  ...
  static Future<Map<String, dynamic>> dmlToSalesforce({
    ...
    else if(opType == Constants.INSERT || opType == Constants.UPDATE){
  ...
  static Future<Map<String, dynamic>> _insertToSalesforce(String objAPIName, List<Map<String, dynamic>> fieldNameValuePairs, int batchCount) async{
    ...
    try{
      resp = await http.post(
        Uri.parse(SalesforceUtil.generateEndpointUrl(opType : Constants.INSERT, objAPIName : objAPIName)), // both are required params
        headers: generateLoggedInRequestHeader(),
        body: jsonEncode(SalesforceUtil.generateBody(opType : Constants.INSERT, objAPIName : objAPIName, fieldNameValuePairs : fieldNameValuePairs, batchCount : batchCount)),
      );
  ...
  static Future<Map<String, dynamic>> _updateToSalesforce(String objAPIName, List<Map<String, dynamic>> fieldNameValuePairs, int batchCount) async{
    ...
    try{
      dynamic resp = await http.patch(
        Uri.parse(SalesforceUtil.generateEndpointUrl(opType : Constants.UPDATE, objAPIName : objAPIName)), // required param is opType
        headers: generateLoggedInRequestHeader(),
        body: jsonEncode(SalesforceUtil.generateBody(opType : Constants.UPDATE, objAPIName : objAPIName, fieldNameValuePairs : fieldNameValuePairs, batchCount : batchCount)),
      );
